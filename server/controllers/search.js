...
const Search = async (req, res, next) => {
  ...
  if (data.userid != "undefined") {
    ...
    if (data.area.length != 0) {
      ...
      query =
        `SELECT U.user_id,CONCAT(firstname, ' ', lastname) AS username,I.*,C.cate_name,A.* FROM item I join category C on C.cate_id=I.cate_id join user_item UI on I.item_id=UI.item_id join user U on U.user_id=UI.user_id  join area A on A.area_id=U.area_id  where I.availability='available' and UI.user_id!=${data.userid}` +
        name +
        max +
        min +
        cate;
