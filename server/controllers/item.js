...
const viewItem =async (req,res,next)=>
{
    ...
    {
        let [result1] = await conn.execute("SELECT I.*,C.cate_name FROM item I join category C on C.cate_id=I.cate_id join user_item U on U.item_id=I.item_id WHERE U.user_id=?", [
...
const addItem = async (req,res,next)=>
{
    ...
    const check_itemName = async (item_name,user_id) =>{
        let [result1] = await conn.execute("SELECT U.user_id FROM user_item U inner join item I on I.item_id=U.item_id WHERE I.item_name=? and U.user_id=?", [
            item_name,user_id
          ]);
        if (result1.length >0) {
            return false;
...
const updateItem = async (req,res,next)=>
{
    ...
    const check_itemid = async (item_id,user_id) =>{
        let [result1] = await conn.execute("SELECT U.user_id FROM user_item U inner join item I on I.item_id=U.item_id WHERE I.item_id=? and U.user_id=?", [
            item_id,user_id
          ]);
        if (result1.length >0) {
            return false;
